...
for ind=1:length(sweepTable)
    ...
    delta = pulse_param{8};
   tau3 = 3*tau+pulse_param{1};
    %tau3 = 3*(tau+pulse_param{1}) -pulse_param{1} ;
   % l = pulse_param{7};
    %% Setting up sequence construction
    
    U_1=[tau3+2*delta,2*(tau3+2*delta),tau3+2*delta];
    U0=[tau-delta,2*(tau-delta),tau-delta];
    %U1=[tau+2e-9,2*(tau+2e-9),tau+2e-9];
    U1=[tau+delta,2*(tau+delta),tau+delta];
    U2=[tau3-2*delta,2*(tau3-2*delta),tau3-2*delta];
    delay={};
    count=0;
    for j=1:p3(l)
        count=count+1;
    ...
    %delay2=repmat(delay,1,4*n/(p(l)+q(l)));%% k is XY8-k
    delay2=repmat(delay,1,2*n/(m1-p3));%% k is XY8-k
    clear delay;
    delay=delay2;
    ...
    npoints = bufferBeg  +(16*n)*round(pulse_param{1}/dt) + (26*n)*round(pulse_param{5}/dt) + bufferEnd;
