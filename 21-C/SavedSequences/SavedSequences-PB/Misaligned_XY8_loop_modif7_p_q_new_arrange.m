...
load peak_samples.mat; %%%%%%%%%%%%%% peak samples for L=4:1:26
...
pause(0.01);   
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p(l)) '_' num2str(q(l))  '_new_arrange.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
%for j=1:size(p,2)
    if p(l)<q(l)
        fp = ceil(q(l)/p(l));
        pp = floor(q(l)/fp);
        if abs(p(l)-m1)>abs(pp - max((p(l)-pp),(q(l)-pp*fp)))
    ...
    elseif p(l)>=q(l)
        fp = ceil(p(l)/q(l));
        qp = floor(p(l)/fp);
        if abs(q(l)-m2)>abs(qp-max((q(l)-qp),(p(l)-qp*fp)))
            for j=1:m2
                count = count+1;
                delay{count}=makeU0(fid);
            end
