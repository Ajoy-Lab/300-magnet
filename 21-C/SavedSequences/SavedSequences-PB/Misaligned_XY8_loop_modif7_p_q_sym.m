p=[1 1 2 1 3 1 5 1 7 11 3 5 5 7];
q=[1 2 1 3 1 5 1 7 1 1 5 3 7 5];

for l=1:length(p)
k=6;
   
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p(l)) '_' num2str(q(l)) '_sym.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
    ...
    fprintf(fid,'%s \n',str);
    fprintf(fid,'%s \n',[ 'p = float(' num2str(p(l)) ',0,500)']);
    fprintf(fid,'%s \n',[ 'q = float(' num2str(q(l)) ',0,500)']);
...
<instructions>
    ...
    for j=1:8*k
        if mod(j,4*(p(l)+q(l)))>=1&&mod(j,4*(p(l)+q(l)))<=2*p(l)-1||mod(j,4*(p(l)+q(l)))>=2*(p(l)+2*q(l))+1&&mod(j,4*(p(l)+q(l)))<=4*(p(l)+q(l))-1||mod(j,4*(p(l)+q(l)))==0
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
            %}
        elseif mod(j,4*(p(l)+q(l)))==2*p(l)||mod(j,4*(p(l)+q(l)))==2*p(l)+4*q(l)
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta+(p+q)*2e-9,2e-9));
            %}
        elseif mod(j,4*(p(l)+q(l)))>=2*p(l)+1&&mod(j,4*(p(l)+q(l)))<=2*(p(l)+2*q(l))-1
            str_spacing{j}=verbatim;
            %{
