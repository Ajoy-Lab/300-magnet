...
k=6;
    ...
    [fid, msg] = fopen(['D:\QEG2\21-C\SavedSequences\SavedSequences-PB\Misaligned_XY8_mod6_12_test.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
<instructions>
    ...
    for j=1:8*k
        if mod(j,6)==1
            ...
            PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
            %}
        elseif mod(j,6)==2||mod(j,6)==0
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta+6e-9,2e-9));
            %}
        elseif mod(j,6)==3||mod(j,6)==4||mod(j,6)==5
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta+12e-9,2e-9));
            %}  
        end
       ...
       str_spacing{j}=verbatim;
       %{
       PSeq = wait(PSeq,round2(tau+delta+6e-9,2e-9));
       %}
       end
    end 
