...
for k = 1:40
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
<instructions>
    ...
    fprintf(fid,'%s \n',['PSeq = set_BNC2(PSeq, BNC_width + ' num2str(1+1.35*k) 'e-6,BNC_ampl,BNC_delay);']);
    ...
    fprintf(fid,'%s \n',['PSeq = wait(PSeq,' num2str(5+0.5*k) 'e-6);']);
    ...
    if k==1
        ...
        PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
        PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
        PSeq = wait(PSeq,round2(2*tau,2e-9));
        PSeq = ttl_pulse(PSeq, 2, length_pi_X);
        PSeq = wait(PSeq,round2(2*tau-2*delta,2e-9));
        PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
        PSeq = wait(PSeq,round2(2*tau + 2e-9,2e-9));
        PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
        PSeq = wait(PSeq,round2(2*tau+2*delta+ 2e-9,2e-9));
        PSeq = ttl_pulse(PSeq, 2, length_pi_X);
        PSeq = wait(PSeq,round2(2*tau+ 2e-9,2e-9));
        PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
        PSeq = wait(PSeq,round2(2*tau-2*delta+ 2e-9,2e-9));
        PSeq = ttl_pulse(PSeq, 2, length_pi_X);
        PSeq = wait(PSeq,round2(tau-delta+ 2e-9,2e-9));
    ...
    fprintf(fid,'%s \n',['PSeq = wait(PSeq,' num2str(5+0.35*k) 'e-6);']);
