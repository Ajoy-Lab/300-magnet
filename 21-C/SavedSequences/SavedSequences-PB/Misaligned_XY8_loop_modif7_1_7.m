...
k=6;
    ...
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p) '_' num2str(q)  '.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
    ...
    <variables>
    %%p=1,q=7, tau=16 ns
    %%the normal peak appears at delta=14 and the error peak appears at delta=0
