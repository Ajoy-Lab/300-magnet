...
for k = 1:30
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_ap' num2str(k) '.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
<instructions>
    ...
    for j=1:k
        ...
        PSeq = wait(PSeq, tau);
    PSeq = ttl_pulse(PSeq, 2, length_pi_X);
    PSeq = wait(PSeq,2*tau+4e-9);
    PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
    PSeq = wait(PSeq,2*tau+8e-9);
    PSeq = ttl_pulse(PSeq, 2, length_pi_X);
    PSeq = wait(PSeq,2*tau+4e-9);
    PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
    ...
    for j=1:k
        ...
        PSeq = wait(PSeq, tau);
    PSeq = ttl_pulse(PSeq, 2, length_pi_X);
    PSeq = wait(PSeq,2*tau+2e-9);
    PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
    PSeq = wait(PSeq,2*tau+4e-9);
    PSeq = ttl_pulse(PSeq, 2, length_pi_X);
    PSeq = wait(PSeq,2*tau+6e-9);
    PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
    PSeq = wait(PSeq,2*tau+8e-9);
    PSeq = ttl_pulse(PSeq, 4, length_pi_Y);
    PSeq = wait(PSeq,2*tau+6e-9);
