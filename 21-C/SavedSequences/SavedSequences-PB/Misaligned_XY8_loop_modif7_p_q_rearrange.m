...
k=6;
    ...
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p(l)) '_' num2str(q(l))  'rearrange.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
<instructions>
    ...
    %%%% loop the spacing according to the combination of p and
    %%%% q,(U0U1)^p(U1)^q-p
      if p(l)<q(l)
      for j=1:8*k
        if j<=4*p(l)
            if mod(j,4)==0||mod(j,4)==2
          ...
          for j=1:8*k
              if j<=2*(p(l)-q(l))-1
                  str_spacing{j}=verbatim;
                    ...
                    PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
                    %}
              elseif mod(j-2*(p(l)-q(l)),4)==0||mod(j-2*(p(l)-q(l)),4)==2
                  str_spacing{j}=verbatim;
                    %{
                    PSeq = wait(PSeq,round2(2*tau+2*delta+(p+q)*2e-9,2e-9));
                    %}
              elseif  mod(j-2*(p(l)-q(l)),4)==1
                  str_spacing{j}=verbatim;
                    %{
                    PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
                    %}
              elseif mod(j-2*(p(l)-q(l)),4)==3
                  str_spacing{j}=verbatim;
                    %{
                    PSeq = wait(PSeq,round2(2*tau+2*delta+(p+q)*4e-9,2e-9));
                    %}  
