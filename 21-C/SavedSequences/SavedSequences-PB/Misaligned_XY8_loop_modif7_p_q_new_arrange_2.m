...
load peak_samples_27_40_2.mat; %%%%%%%%%%%%%% peak samples for L=4:1:26
countk=0;
kvec=[60:-2:52];
sizek=size(kvec,2);
...
%   q=peak_samples{k-3}(:,1);
%   p=peak_samples{k-3}(:,2);

k
countk=countk+1;

p=peak_p{sizek-countk+1}(1:end);
q=peak_q{sizek-countk+1}(1:end);
p1=peak_p1{sizek-countk+1}(1:end);
p2=peak_p2{sizek-countk+1}(1:end);
q1=peak_q1{sizek-countk+1}(1:end);
q2=peak_q2{sizek-countk+1}(1:end);
m1=peak_m1{sizek-countk+1}(1:end);
...
pause(0.05);   
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p(l)) '_' num2str(q(l))  '_new_arrange_2.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
delay2=repmat(delay,1,4*k/(p(l)+q(l)));
clear delay ;
delay=delay2;
...
for j=1:size(delay,2)
 ...
 fprintf(fid,'%s \n',['PSeq = ttl_pulse(PSeq, 2, length_pi2_X);']);
    ...
    fclose(fid);
 clear delay delay2 sequence count    
end
