...
k=6;
    ...
    [fid, msg] = fopen(['D:/QEG/21-C/SavedSequences/SavedSequences-PB/Misaligned_XY8_mod' num2str(k) '_' num2str(p(l)) '_' num2str(q(l))  '.xml'], 'w');
    if fid == 1
        error(msg);
    end
    fprintf(fid, '%s \n');
...
<instructions>
    ...
    for j=1:8*k
        if mod(j,2*(p(l)+q(l)))>=1&&mod(j,2*(p(l)+q(l)))<=2*p(l)-1
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta,2e-9));
            %}
        elseif mod(j,2*(p(l)+q(l)))==2*p(l)||mod(j,2*(p(l)+q(l)))==0
            str_spacing{j}=verbatim;
            %{
            PSeq = wait(PSeq,round2(2*tau+2*delta+(p+q)*2e-9,2e-9));
            %}
        elseif mod(j,2*(p(l)+q(l)))>=2*p(l)+1&&mod(j,2*(p(l)+q(l)))<=2*(p(l)+q(l))-1
            str_spacing{j}=verbatim;
            %{
