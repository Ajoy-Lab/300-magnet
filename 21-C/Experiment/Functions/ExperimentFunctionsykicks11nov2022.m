...
%                                       Sage_write(2);
                                if handles.Array_PSeq{1}.Channels(38).Enable && ~handles.Imaginghandles.ImagingFunctions.interfaceDataAcq.hasAborted
                                    if aux<handles.ExperimentalScan.vary_points(1)
                                        ...
                                        if sequence_type==10
                                          pw3=handles.Array_PSeq{helper_scan,aux+1}.Channels(45).Frequency;
                                        end
...
%                                           make_pw_nmr_3_rof(pw,2);
                                    elseif sequence_type==10
                                        ...
                                        tacq_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).Phase;
                                        gain_current=handles.Array_PSeq{helper_scan,aux}.Channels(38).FreqIQ;
                                        pw3_current=handles.Array_PSeq{helper_scan,aux}.Channels(45).Frequency;



                                        [nc,Tmax]= make_pw_nmr_ykick(pw_current,tacq_current,gain_current,pw3_current);
                                        pause(0.1);
                                        make_pw_nmr_ykick(pw,tacq,gain,pw3);
...
%                                     tacq=32;
                                    if length(handles.Array_PSeq{1}.Channels)>=44 && handles.Array_PSeq{1}.Channels(44).Enable
                                        ...
                                        if sequence_type==10
                                           Sage_write(['2,',num2str(pw_current),',',num2str(nc),',',num2str(Tmax),',',num2str(tacq_current),',',num2str(pw3_current)]); %write to Sage for initializing readout 
                                        end
                                    end
                                    disp('writing sage parameters');
