...
%                 tof = -1000*cmdBytes(2);
                ...
                ch=1;
                initializeAWG(ch);
                generatePulseSeqIQ(ch, amps, frequencies, lengths, phases, spacings, reps, markers, markers2, trigs);
...
%                 readLen = 97920; % for tacq=96
                ...
                %assert(rc.ErrCode == 0);
                rc = inst.SendScpi(sprintf(':DIG:TRIG:AWG:TDEL %f', lengths(2) + 12e-6));
                assert(rc.ErrCode == 0);
                ...
                %rc = inst.SendScpi(':DIG:TRIG:SOURCE EXT'); %digChan
                %assert(rc.ErrCode == 0);
                %inst.SendScpi(':DIG:TRIG:TYPE GATE');
                %rc = inst.SendScpi(':DIG:TRIG:SLOP NEG');
                %assert(rc.ErrCode == 0)
                %rc = inst.SendScpi(':DIG:TRIG:LEV1 0.3');
                %assert(rc.ErrCode == 0)
                %rc = inst.SendScpi(sprintf(':DIG:TRIG:DEL:EXT %g', 5e-6));
                %assert(rc.ErrCode == 0)
...
%                     
                    ...
                    assert(rc == 0);
                    samples2 = double(netArray2); %get the data (1s chunk)
...
function generatePulseSeqIQ(ch, amps, frequencies, lengths, phases, spacings, reps, markers1,markers2, trigs)
global inst
global sampleRateDAC
global interp
global sampleRateInterp
    ...
    padding = round2(12e-6*sampleRateDAC,2);
    myMkr1 = uint8(zeros(length(dacWave)/4, 1) + state1);
%         myMkr1 = [myMkr1', uint8(zeros(padding,1))'];
    myMkr2 = uint8(zeros(length(dacWave)/4, 1) + state2);
%         myMkr2 = [uint8(zeros(padding,1))', myMkr2'];
    myMkr = myMkr1 + 2*myMkr2;
%         myMkr = myMkr';
    inst.SendScpi(sprintf(':INST:CHAN %d',ch));
    ...
    %inst.SendScpi(':MARK:VOLT:LEV 0.0')
    inst.SendScpi(':MARK:VOLT:OFFS 0.5');
    inst.SendScpi(':MARK:STAT ON');
...
fprintf('pulse sequence generated')
    ...
    downLoadIQ(ch, 1, segMat{1,1}, segMat{2,1}, 0,0, 1);
    for y=1:numPulses
       downLoadIQ(ch, 2*y, segMat{1,2*y}, segMat{2,2*y}, 0,0, 1);
       downLoadIQ(ch, 2*y+1, segMat{1,2*y+1}, segMat{2,2*y+1}, markers1(y),markers2(y), 1);
   end
