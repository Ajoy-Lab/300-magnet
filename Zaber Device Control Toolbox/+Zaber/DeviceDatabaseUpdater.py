...
def createArgParser():
    ...
    parser.add_argument("--devices", dest = "devices", action="store_true", default = False, help = "Optional: Update the device database .mat file.")
    parser.add_argument("--enums", dest = "enums", action="store_true", default = False, help = "Optional: Update the binary code enumerations.")
...
def getUnits(aCursor, aDimensionTable, aProductId):
    ...
    aCursor.execute("SELECT * FROM Console_BinaryCommands2 WHERE ProductId = " + str(aProductId) + " AND Command = 21 AND ParameterUnitScale NOT NULL AND ParameterUnitFunction = '" + function + "';")
    rows = aCursor.fetchall()
    if (len(rows) > 0):
